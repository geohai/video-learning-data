### Adapted from https://github.com/facebookresearch/Ego4d/blob/main/ego4d/research/clep/dataset.py
...
def create_egoexo_multiview_dset(
    config: TrainConfig, use_ego_sent: bool, ego_only: bool
) -> Tuple[Dataset, Tensor,]:
    """
    Loads the egoxo dataset and returns a dataset and the associated
    sentence embeddings for each class (as a single tensor, where class_idx ==
    idx in the tensor).
    """
    val_df = pd.read_csv(config.pre_config.egoexo.set_path)
    ...
    sent = torch.load(
        os.path.join(
            ...
            config.pre_config.egexo.out_label_path,
        )
    )
    ...
    feature_hdf5_path = os.path.join(
        config.pre_config.root_dir,
        config.pre_config.egoexo.out_path,
    )
    id_classes_pairs = []
...
### TODO: change the below for egoexo
...
#             config.pre_config.root_dir, config.pre_config.egoexo_narr.metadata_out_path
#         )
...
#             os.path.join(
#                 config.pre_config.root_dir,
#                 config.pre_config.egoexo_features.hdf5_path,
#             )
#         )
